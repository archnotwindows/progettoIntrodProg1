...
* * Questo file gestisce l'interfaccia utente per la scelta della missione e il while che regola:
* - avanzamento delle stanze
* - accesso al negozio
...
* * La funzione verifica quali missioni sono state completate per mostrare dinamicamente le opzioni.
* Una volta selezionata una missione, entra in un loop che la gestisce fino al completamento.
...
void Menu(Giocatore* giocatore_ptr) {   
    ...
    * - true solo e solo se tutte le missioni sono state completate.
    * è necessario per sbloccare la missione finale.
    */
    ...
    int opzione_numero = 1; // Contatore per numerare le opzioni del menu
    
    /**
    * @brief Array di mappatura opzioni -> ID missione.
    * Poiché le missioni spariscono dal menu una volta completate, l'opzione "1" non corrisponde sempre alla missione 1.
    * Questo array mappa la scelta dell'utente (indice array) all'ID effettivo della missione (valore).
    ...
    */
    ...
    /* Se tutte le missioni base sono completate, mostra Missione Finale */
    if (tutte_completate) {      
        printf("\n*** MISSIONE FINALE ***\n");
