...
void Inventario(Giocatore* g) {
    ...
    if (!g->ha_spada && !g->ha_spada_eroe && !g->ha_armatura && !g->ha_chiave) {     // se le variabili degli oggetti sono tutte false => lo zaino è vuoto 
        printf("  (Zaino vuoto)\n");                                               
    } else {
    ...
    printf("  - Palude:  %s\n", g->missione_palude_completata ? "[X]" : "[ ]");     // se missione_palude_completata = true scrive [X] altrimenti [ ]
